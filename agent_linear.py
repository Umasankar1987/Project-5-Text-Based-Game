...
def epsilon_greedy(state_vector, theta, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        q_val = theta @ state_vector
        action_index, object_index = index2tuple(np.argmax(q_val))
...
def linear_q_learning(theta, current_state_vector, action_index, object_index,
    ...
    q_value_next = theta @ next_state_vector
    max_q_next = np.max(q_vale_next)
    index = tuple2index(action_index, object_index)
    q_value1 = q_value[tuple2index(action_index, object_index)]
    y = reward + GAMMA*max_q_next*(1 - terminal)
    theta[index] += ALPHA*(y - q_value1)*current_state_vector  # TODO Your update here
# pragma: coderesponse end
