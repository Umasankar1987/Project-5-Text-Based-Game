...
def epsilon_greedy(state_1, state_2, q_func, epsilon):
    ...
    if np.random.rand() < epsilon:
        ...
        q_val = q_func[state_1, state_2, :, :]        
        (action_index, object_index) = np.unravel_index(np.argmax(q_val, axis=None),
                                          q_val.shape)
...
def tabular_q_learning(q_func, current_state_1, current_state_2, action_index,
    ...
    if terminal:
        ...
        maxq_nextstate = np.max(q_func[next_state_1,next_state_2,:,:])
        
        
    q_func[current_state_1, current_state_2, action_index,
           object_index] = (1 - ALPHA)*q_func[current_state_1, current_state_2, action_index, object_index] + ALPHA*(reward + GAMMA*maxq_nextstate) # TODO Your update here
